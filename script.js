...
if (!quest) {
	console.log("You have no incomplete quests!");
} else {
	...
	if (taskName === "WATCH_VIDEO" || taskName === "WATCH_VIDEO_ON_MOBILE") {
		...
		if (!isApp) {
			console.log("This no longer works in the browser for non-video quests. Use the desktop app to complete the quest for", applicationName, "!");
		} else {
			api.get({ url: `/applications/public?application_ids=${applicationId}` }).then(res => {
				...
				console.log(`🎮 Your game has been spoofed for ${applicationName}. Please wait about ${Math.ceil((secondsNeeded - secondsDone) / 60)} minutes.`);
			});
		}
		...
		if (!isApp) {
			...
			console.log(`📡 Your stream has been spoofed for ${applicationName}. Stream any window in VC for about ${Math.ceil((secondsNeeded - secondsDone) / 60)} minutes.`);
			console.log("Remember: you need at least one more person in the VC!");
		}
